          ...
          <thead>
            <tr className="border-b border-ink-100">
              {columns.map((c) => (
                <th
                  key={c.key}
                  className={`px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-ink-400 ${c.className || ''}`}
                >
                  {c.label}
                </th>
              ))}
              ...
              rows.map((row, i) => (
                <tr
                  ...
                  className={`border-b border-ink-100 last:border-0 ${
                    onRowClick ? 'cursor-pointer hover:bg-ink-50/60' : 'hover:bg-ink-50/40'
                  } transition-colors`}
                >
                  {columns.map((c) => (
