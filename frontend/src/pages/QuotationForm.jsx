                ...
                <tbody>
                  {cart.map((line, i) => (
                    <tr key={i} className="border-b border-ink-100 last:border-0">
                      ...
                      <td className="px-3 py-2.5 text-right"><input className="input input-sm w-24 text-right num" type="number" value={line.unitPrice} onChange={(e) => setLine(i, { unitPrice: Number(e.target.value) })} /></td>
                      <td className="px-3 py-2.5 text-right"><input className="input input-sm w-20 text-right num" type="number" value={line.discount} onChange={(e) => setLine(i, { discount: Number(e.target.value) })} /></td>
                      <td className="px-4 py-2.5 text-right num font-semibold text-ink-900 whitespace-nowrap">{money(Math.max(0, line.quantity * line.unitPrice - (line.discount || 0)), currency)}</td>
          ...
          <div className="space-y-4 lg:sticky lg:top-20">
            <div className="card p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="label">Discount</label><input className="input num" type="number" value={discount} onChange={(e) => setDiscount(e.target.value)} /></div>
            ...
            <div className="card p-5">
              ...
              <button className="btn-primary-gradient btn-lg w-full mt-4" disabled={saving || !cart.length} onClick={submit}>{saving ? <><Spinner className="w-4 h-4" /> Saving…</> : 'Save Quotation'}</button>
            </div>
          </div>
        </div>
      </div>
